...
class App extends Component {
  ...
  componentDidMount() {
    ...
    let jlistTitleIF = [];
    axios
      .get(process.env.REACT_APP_FIREBASE_API_KEY)
      .then((response) => {
      ...
      .then((response) =>
        this.setState({ jlist, jlistTitleIF }, () => this.getIdList())
      );
  }
  ...
  dateInput = (date1, date2) => {
    if (this.state.papersListNoJournals.length) {
      alert(
        'The date input does not work when a journal list is not defined. Click on "Journals" in the toolbar to refine your search or "RESET" to return to the initial view with the curated list.'
      );
    } else {
      ...
      this.setState(
        { inputedDate1: convertedDate1, inputedDate2: convertedDate2 },
        () => this.getIdList()
      );
    }
  };
  ...
  getIdList = () => {
    ...
    if (this.state.jlist.length > 50) {
      const jlistFirst = this.state.jlist.slice(
        0,
        Math.floor(this.state.jlist.length / 2)
      );
      const jlistLast = this.state.jlist.slice(
        Math.floor(this.state.jlist.length / 2)
      );
    ...
    this.setState(
      ...
      () => {
        ...
        this.state.splitjlist.forEach((jlistitem) => {
          ...
          if (!this.state.inputedDate1) {
            axios
              .all([http.get(url), http.get(url2)])
              .then(
                axios.spread((responseUrl1, responseUrl2) => {
                  const idlist = this.state.idlist.concat(
                    responseUrl1.data.esearchresult.idlist
                  );
                  const idlistWeek = this.state.idlistWeek.concat(
                    responseUrl2.data.esearchresult.idlist
                  );
                  this.setState({ idlist, idlistWeek });
                  loop = loop + 1;
                  splitjlist.length === loop && this.addPapers();
                })
              )
              .catch((error) =>
            ...
            axios
              .get(url)
              .then((response) => {
                const idlistInputedDate = this.state.idlistInputedDate.concat(
  ...
  noJournalSearch = () => {
    ...
    axios
      ...
      .then((response) =>
        this.setState(
          {
            idlistNoJournals: response.data.esearchresult.idlist,
            selectedJournals: [],
            newJournals: [],
            inputedDate1: "",
            inputedDate2: "",
      ...
      .catch((error) => this.setState({ apiError: true, loading: false }));
  };
  ...
  addPapers = () => {
      ...
      if (this.state.idlistInputedDate.length > 2000) {
        alert(
          "Your search returned: " +
            ...
            " papers. All papers beyond the 2000th have been removed. If you want to see more papers in that date range, please perform separate searches using closer start and end dates."
        );
    ...
    if (this.state.idlistWeek.length < 350) {
      ...
      for (let i = 0; i < this.state.idlistWeek.length; i += 349) {
        paperListStringWeek.push([
          this.state.idlistWeek.slice(i, i + 349).toString(),
        ]);
      }
    }
    ...
    if (idlistToDisplay.length) {
      ...
      paperListString.forEach((listString) => {
        axios
          .get(
            `https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?db=pubmed&retmode=json&id=${listString}&api_key=${process.env.REACT_APP_PUBMED_API_KEY}`
          )
            ...
            Object.keys(myObj).forEach((key) => {
              ...
              if (key !== "uids") {
                ...
                this.state.jlistTitleIF.forEach((jlistItem) => {
                  if (
                    jlistItem.title.toLowerCase() ===
                      myObj[key].fulljournalname.toLowerCase() ||
                    jlistItem.title.toLowerCase() ===
                      myObj[key].source.toLowerCase()
                  ) {
            ...
            if (loop === paperListString.length) {
              if (this.state.idlistNoJournals.length) {
                paperList.length === idlistToDisplay.length &&
                  this.setState({
                    papersListNoJournals: paperList,
                    loading: false,
                  });
              } else if (this.state.idlistInputedDate.length) {
                paperList.length === idlistToDisplay.length &&
                  this.setState({
                    papersListInputedDate: paperList,
                    loading: false,
                  });
              } else if (this.state.idlist.length) {
                paperList.length === idlistToDisplay.length &&
            ...
            .then((response2) => {
              this.state.idlistWeek.forEach((id) => {
                ...
                this.state.jlistTitleIF.forEach((jlistItem) => {
                  if (
                    jlistItem.title.toLowerCase() ===
                      response2.data.result[id].fulljournalname.toLowerCase() ||
                    jlistItem.title.toLowerCase() ===
                      response2.data.result[id].source.toLowerCase()
                  ) {
  ...
  filterJournals = (selectedJournals, newJournals) => {
    let jlist = [];
    let filteredJournalList = this.state.journals.filter(
      (journal) => !selectedJournals.includes(journal.title)
    );
    let filteredJournalListTitles = filteredJournalList.map(
  ...
  render() {
    ...
    if (this.state.loading) {
      ...
      papersDisplay = <ApiError apiReset={this.apiReset} />;
    }
      ...
      <div className="App">
        ...
        <Switch>
          <Route
            ...
            render={() => (
              <>
                <WelcomeBanner country={this.state.country} />
                <DateInput
          ...
          <Route exact path="/contact" render={() => <EmailForm />} />
          <Route exact path="/admin/add" render={() => <AdminAddJournal />} />
          <Route
            exact
            path="/admin/update"
            render={() => <AdminUpdateJournal />}
          />
          <Route
            exact
            path="/admin/delete"
            render={() => <AdminDeleteJournal />}
          />
        </Switch>
      </div>
    );
